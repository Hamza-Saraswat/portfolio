...
<StepList steps={[
  { title: 'Cut the quoted history', body: 'Thread history is stripped before the model sees it, so a question from three weeks ago cannot bleed into a card labeled "latest update."' },
  { title: 'Generate', body: 'The model rewrites the update in a specified voice, with internal jargon banned, and with the rule that it must sanitize rather than suppress: an email about an $18.40 payment mismatch becomes a note about a small mismatch we asked the customer to confirm. Useful, but with the number gone.' },
