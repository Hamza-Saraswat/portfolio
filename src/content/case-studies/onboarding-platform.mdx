...
tagline: Salesforce closed-won to a configured account, self-serve, with an orchestrator that never strands the customer.
...
attribution: Three repositories. I led development on the wizard (86 of 107 commits, alongside the PM who owns the product), and built the link-generation API and the analytics dashboard.
order: 5
...
related: ['ai-adoption', 'customer-status-pages']
---
...
<Figure
  ...
  caption="Closed-won in Salesforce produces a personalised link. The customer never sees a login screen."
/>

The flow starts in Salesforce. When a deal closes, an automation calls a small API that mints a session and returns a personalised link. The customer opens it, the token is exchanged for a session cookie, and they walk through the setup modules — each one checkpointing to the database as they go, so a closed tab is not a lost afternoon.
...
**Reconcile schema drift automatically.** Repository migrations and the deployed database drifted apart, and reconciling them by hand was tedious and error-prone. A check in the pipeline comparing the two would have caught it the day it happened.

**Give operators a real interface to the failure queue sooner.** Fail-soft is only as good as the process that drains the queue. Audit rows shipped well before anything designed for reading them, which for a while meant the safety net depended on someone running a query.

**Instrument for outcomes at launch, not after.** The dashboard measures the funnel — where sessions stall, which jobs fail. It does not measure whether onboarding got faster than the manual process it replaced, because nobody captured a clean baseline before the change. That is why this page has no before-and-after number, and it is the thing I would fix first.
